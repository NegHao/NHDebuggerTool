...
- (void)layoutSubviews
{
    ...
    if (_unitLabel.text.length > 0) {
        _unitLabel.frame = CGRectMake(viewWidth * 0.7, CGRectGetMinY(_numberLabel.frame) - 0.5, _unitLabel.font.pointSize*3, _unitLabel.font.pointSize);
    } else{
        _unitLabel.frame = CGRectZero;
    }
    
    _explainLabel.frame = CGRectMake((viewWidth - explainLabelWidth)/2, viewHeight - explainLabelHeight - 10, explainLabelWidth, explainLabelHeight);
}

-  (void)setFirstWaveColor:(UIColor *)firstWaveColor{
    [_waterWaveView setFirstWaveColor:firstWaveColor];
}

- (void)setSecondWaveColor:(UIColor *)secondWaveColor{
    [_waterWaveView setSecondWaveColor:secondWaveColor];
}
